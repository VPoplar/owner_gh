...
object Spark01_RDD_File {
  def main(args: Array[String]): Unit = {
    ...
    //val rdd: RDD[String] = sc.textFile("datas")  也可以读取整个目录下得文件
    //val rdd: RDD[String] = sc.textFile("datas/1*.txt")  也可以使用通配符
    //val rdd = sc.textFile("hdfs://linux1:8020/test.txt")  也可以是分布式存储系统路径：HDFS
